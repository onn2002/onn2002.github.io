...
let logoYPosition; // Y-position for the logo animation
let logoYDirection = 1; // Direction for the logo movement
...
function setup() {
  ...
  // Set initial Y-position for the logo
  logoYPosition = height / 4;
}
...
function animateLogo() {
  // Bounce the logo up and down
  logoYPosition += logoYDirection * 2; // Change the speed as needed

  // Reverse direction if the logo reaches certain boundaries
  if (logoYPosition > height / 2 || logoYPosition < height / 4) {
    logoYDirection *= -1;
  }

  // Display the logo
  imageMode(CENTER);
  image(logo, logoDiameter / 2, logoYPosition, logoDiameter, logoDiameter); // Positioned on the left
}
